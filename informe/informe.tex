...
Ahora verifiquemos que los nodos seleccionados en el caso cuando la métrica de todos los rectángulos es 0. En estos casos el algoritmo anterior toma los de mayor área, tales que si hay 2 de igual cantidad de zonas sin pintar se coloca primero el de menor peso. sea p el mayor rectángulo del subconjunto de rectángulos que cumplen $\{ r \in$ $r_{2,i} \mid métrica(r)=0 \}$.\\
...
El problema puede ser visto como un Digrafo cíclico(si no hay ciclios no tiene sentido el ejercicio), donde las salas del laberinto serían los nodos, y los pasillos los arcos. Por lo que el problema se resume en encontrar la menor cantidad de aristas a quitar para hacer el grafo acíclico.  
	...
	\end{itemize}
\subsection{Comprobación en tiempo polinomial}
